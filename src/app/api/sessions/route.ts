...
export async function GET(request: Request) {
  ...
  if (user?.role === "ADMIN") {
    ...
    const clsRes  = await fetch(
      `${BASE_API_URL}/attendance/classrooms?size=200`,
    ...
    const results = await Promise.all(
      classrooms.map((c) =>
        fetch(
          `${BASE_API_URL}/attendance/sessions/classrooms/${c.id}?from=${from}&to=${to}&size=50`,
          { cache: "no-store", headers: { Cookie: cookieHeader } }
        )
          .then((r) => r.json())
  ...
  const res = await fetch(
    `${BASE_API_URL}/attendance/sessions/teachers/${teacherId}/upcoming?page=${page}&size=${size}`,
    { cache: "no-store", headers: { Cookie: cookieHeader } }
  );
