...
async function fetchAllClassrooms(): Promise<Classroom[]> {
  try {
    const res = await fetch(`${BASE_API_URL}/classrooms?size=100`, { cache: "no-store" });
    if (!res.ok) return [];
...
async function fetchTeacherClassrooms(teacherId: string): Promise<Classroom[]> {
  try {
    const [schedRes, clsRes] = await Promise.all([
      fetch(`${BASE_API_URL}/schedules/teachers/${teacherId}?size=100`, { cache: "no-store" }),
      fetch(`${BASE_API_URL}/classrooms?size=100`, { cache: "no-store" }),
    ]);
